...
{ 
    ...
    protected void Repeater1_ItemCommand(object sender, RepeaterCommandEventArgs e)
    {
        ...
        {
            i = sql.ExecuteSql("update News set status='Deactive' where id='" + e.CommandArgument.ToString() + "'");
    ...
    void get_item()
    {
        ...
        DataTable dt = sql.GetDataTable("select * from News where status='Active'");
        ViewState["emp"] = dt;
    ...
    {
        DataTable dt = sql.GetDataTable("select * from News where Notice_title='" + txtnotice_title.Text + "' and Status='Active'");
        if (dt.Rows.Count > 0)
        {
        ...
        else
        {
            ...
            int i = 0;
            i = sql.ExecuteSql("insert into News (Notice_title,Notice_Date,Notice_details,addeddate,addedby,status) values ('" + txtnotice_title.Text + "','" + txtnotice_date.Text + "','" + txtnotice.Text+ "','" + System.DateTime.UtcNow.AddMinutes(330).ToString() + "','Admin','Active')");
