...
public partial class Admin_TcForm_update : System.Web.UI.Page
{
    string refr;
    ...
    protected void getlogin()
    {
        ...
        dt = sql.GetDataTable("select * from Apply_tc where student_id='STD00007'  ");
        if (dt.Rows.Count > 0)
        {
    ...
    {
        string number_value = "";
        DataTable dt1 = sql.GetDataTable("select max(id)+1 from Apply_Tc");
    ...
    protected void btnupdate_Click(object sender, EventArgs e)
    {
        ...
        int i = sql.ExecuteSql("update Apply_tc set Activity='Genreted',Reg_no='" + txtregno.Text + "',Student_name='" + txtstudent_num.Text + "', Father_name='" + txtfathername.Text + "',mother_name='" + txtmother.Text + "',mobile='" + mobile.Text + "', Stream='" + txtstream.Text + "',Honours='" + txtsub.Text + "', Roll_no='" + txtrollno.Text + "',Session_from='" + txtsessionfrom.Text + "', Session_to='" + txtsessionto.Text + "',Passing_year='" + txtPas_year.Text + "',division='" + txtdivision.Text + "', Address='" + txtaddress.Text + "', Date_of_leave_college='" + txtleavingdate.Text + "',Dob='" + txtnotice_date.Text + "',Condition='" + txtbehaviour.Text + "', updateddate='" + System.DateTime.Now.ToString("dd/MM/yyyy") + "',Tc_id='" + Tc_id + "' where Student_id='STD00007'");
        if (i > 0)
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Record update ');window.location.href='Download_tc.aspx?id=" + refr + "';", true);
            // txtreg.Text = "";
        }
        else
        {
