...
public partial class Admin_ApplyTc : System.Web.UI.Page
{
    ...
    protected void save_Click(object sender, EventArgs e)
    {
        ...
        int i = 0;
        i = sql.ExecuteSql("insert into Apply_tc (Reg_no,Roll_no,Student_name,Father_name,mother_name,Mobile,Aadhar,Dob,email,gender,Session_from,Session_to,Stream,Honours,Fee_recipt,Marksheet,addeddate,addedby,Passing_year,division,address,status,Activity,Student_id,Photo) values ('" + txtregno.Text + "','" + txtrollno.Text + "','" + txtstudent_num.Text + "','" + txtfathername.Text + "','" + txtmother.Text + "','" + mobile.Text + "','" + txtaadhar_no.Text + "','" + email.Text + "','" + txtnotice_date.Text + "','" + ddlgender.SelectedValue + "','" + txtsessionfrom.Text + "','" + txtsessionto.Text + "','" + txtstream.Text + "','" + txtsub.Text + "','" + img1 + "','" + img2 + "','" + System.DateTime.UtcNow.AddMinutes(330).ToString() + "','Student','" + txtPas_year.Text + "','" + txtdivision.Text + "','" + txtaddress.Text + "','Active','Pending','" + Request.Cookies["user_id"].Value + "','" + img3 + "')");
    ...
    protected void getapply()
    {
        ...
        dtt = sql.GetDataTable("select * from Apply_tc where student_id='" + Request.Cookies["user_id"].Value + "'  ");
        if (dtt.Rows.Count > 0)
        {
        ...
        public void payment(string id)
    {
        ...
        string ru = "https://www.anscollege.ac.in/Student/TcRecipet.aspx";
